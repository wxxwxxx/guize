      ...
      function ssr2ss(proxies) {
        ...
        let res = proxyList.map(proxy => {
          if (/=\s*shadowsocksr/.test(proxy)) {
            ...
            return proxy.replace(/=\s*custom/g, '= ss').replace(/\s*(none|aes|rc4|salsa20|chacha20)[^,$]+/g, 'encrypt-method=$&').replace(/\wenwenai2/g, 'password=$&').replace(/,\s*[https://github.com/lhie1/Rules/blob/master/SSEncrypt.module?raw=true]/g, '')
          }
