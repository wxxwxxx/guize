...
function makeConf(params) {
    ...
    try {
        let pu = {
            ...
            //urlreject: 'https://raw.githubusercontent.com/lhie1/Rules/master/Auto/REJECT.conf',
        ...
        let onPgs = function(hint) {
            pgs += 0.1 'onProgress' in params && params.onProgress(pgs, hint)
        }
            ...
            function ssr2ss(proxies) {
                ...
                let res = proxyList.map(proxy => {
                    if (/=\s*custom/.test(proxy)) {
                        return proxy.replace(/=\s*custom/g, '= ss').replace(/\s*(none|aes|rc4|salsa20|chacha20)[^,$]+/g, 'encrypt-method=$&').replace(/(\w)(\w+)(U0|ai2)/g, 'password=$&').replace(/\, undefined/g, '')
                    } else if (/=\s*shadowsocksr/.test(proxy)) {
                        return proxy.replace(/=\s*shadowsocksr/g, '= ss').replace(/"/g, '').replace(/\s*(none|aes|rc4|salsa20|chacha20)[^,$]+/g, 'encrypt-method=$&').replace(/(\w)(\w+)(U0|ai2)/g, 'password=$&').replace(/,\s*(protocol|protocol_param|obfs|obfs_param)[^,$]+/g, '')
